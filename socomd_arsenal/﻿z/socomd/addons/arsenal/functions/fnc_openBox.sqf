...
if (is3DEN) then {
    _displayToUse createDisplay QGVAR(socomd_display);
} else {
    [{(_this select 0) createDisplay (_this select 1)}, [_displayToUse, QGVAR(socomd_display)]] call CBA_fnc_execNextFrame;
};
