...
dokeyDown={
   ...
   if ((player getvariable["key",true]) and (_this select 1)  == Keys) exitwith {
	   player setvariable["key",false]; 
	   [_key_delay] spawn {sleep (_this select 0);player setvariable["key",true];
	    }; 
		_r
	};
...
Frontpack={
	...
	[_target,_class,_pack] spawn {
		private ["_target","_class","_packHolder","_pack"];
		_target	= _this select 0;
		_class 	= _this select 1;
		_pack = _this select 2;
