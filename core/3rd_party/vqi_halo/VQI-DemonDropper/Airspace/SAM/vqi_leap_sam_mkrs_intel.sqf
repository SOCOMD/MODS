...
// Grab Alpha
_mkrAlpha = [];
{ _mkrAlpha set [count _mkrAlpha,[_x select 10]]; } forEach aoSAMdata;
if (VQI_HINTS_HALO == 1) then { hint format ["Alpha Data: %1",_mkrAlpha]; };
sleep 1;


// Total Variables in Array
_mkrCount = (count _mkrAlpha) -1;
if (VQI_HINTS_HALO == 1) then { hint format ["COUNT: %1",_mkrCount]; };
sleep 1;
