// Mark and Fadeout Intel from X pos - UAV
// Local or Global ??

private ["_marker","_markers"];


_loiter = 720;	// 1hr @ 5sec loops
...
_logic sideChat "MAP GRID Recieved. Stand by..."; sleep (2 + random 5);
_logic sideChat "Fuel Check... On Station Time: 1hr"; sleep (2 + random 5);
_logic sideChat "UAV Oscar Tango Whiskey, Angels 10"; sleep (2 + random 5);
_logic sideChat "Flight Plan, Air Traffic Logged..."; sleep 5;


// Target Area Object
UAVpos = "Land_PenBlack_F" createVehicle _gpsLOC;

_rTime = round ((120 + random 600) / 60);
_logic sideChat format ["ETA %1 minutes",_rTime];
_flight = (_rTime * 60);
...
sleep _flight;
_logic sideChat "UAV is now ON STATION"; sleep 5;
...
while {_loiter > 0} do {
...

	{
		if (vehicle leader _x == leader _x) then {
			...
			_posUAV = getPos UAVpos;									// GPS LRSR
			
			if (_posUAV distance2D _posXYZ < 1000) then {
...
_logic sideChat "Bingo Fuel. UAV is now OFF STATION, OUT";
