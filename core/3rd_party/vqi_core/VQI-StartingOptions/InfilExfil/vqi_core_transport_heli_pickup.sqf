//
//
if (VQI_CORE_HINTS == 1) then { hint "heli_pickup.sqf"; };
sleep 1;


private ["_gpsLOC","_player","_rngCHX","_tooFAR","_maxNUM","_callSign","_radio","_pos00","_logic","_mapSize","_mapHalf","_mapCent","_rDir","_sLoc","_nameLZ","_mkrLZ","_heli","_posFSP","_smoke","_posSMK","_smkLZ"];

if ( !isNil "coreHELI" ) exitWith { hint "Radio Failure \n or \n Heli already dispatched... \n\n - Try Again Later -"; };	// already exits
...
_logic sideChat "Solid Copy. Bird Inbound. Stand By to MARK LZ. Will Advise..."; sleep 5;
...
if (typeOf coreHELI == "B_Heli_Light_01_F") then { coreHELI allowDamage true; /* coreHELI setCaptive true;*/ coreHELI setObjectTextureGlobal [0,"A3\Air_F\Heli_light_01\Data\heli_light_01_ext_ion_co.paa"]; };
...
*/
	...
	_mkrLZ = createMarker["mkrLZ", _gpsLOC];
    ...
    _pBIS = [_gpsLOC, 0, 100, 25, 0, 0.25, 0] call BIS_fnc_findSafePos; 
	coreLZ = createVehicle ["Land_HeliPadEmpty_F", _pBIS, [], 0, "NONE"];
	sleep 5;
...
*/
	...
	//if ((VQI_CORE_SUPPORT_HELI == 2) || (VQI_CORE_SUPPORT_HELI == 3) || (VQI_CORE_SUPPORT_HELI == 4)) then {
		coreHELI addAction ["--------------"," "];
	...
	waitUntil { sleep 1; (getMarkerPos "mkrLZ") distance2D coreHELI < 3000; };
	_logic sideChat "3 Clicks Out...";


	waitUntil { sleep 1; (getMarkerPos "mkrLZ") distance2D coreHELI < 1000; };
					...
					if ("SmokeShellPurple" in magazines player) then { 
						...
						if ("SmokeShellGreen" in magazines player) then { 
							...
							if ("SmokeShellOrange" in magazines player) then { 
								_logic sideChat "On Site... POP ORANGE SMOKE to confirm!"; _smoke = 6; 
							};
						};
					};
				};
	...
	//----LZ---------------------------------------------------------
	waitUntil { sleep 1; (getMarkerPos "mkrLZ") distance coreHELI < 200; };


	// Smoke/IR
	if (_smoke == 0) exitWith { 
		_logic sideChat "On Site... POP BLUE SMOKE to confirm!"; sleep 10; 
		_logic sideChat "CAN NOT CONFIRM LZ!!!"; _heli move (getMarkerPos "mkrCORE_IN");
		sleep 30;
		
		{ if (vehicle _x != _x) then { deleteVehicle vehicle _x; }; deleteVehicle _x }
		forEach (units (coreHELI select 2));
		sleep 2;
	...
	// check for smoke
	if (getPosASL player select 2 > 0) then {
		if (_smoke == 1) then { waitUntil { sleep 1; count (getMarkerPos "mkrLZ" nearObjects ["SmokeShellBLUE",250]) > 0; }; };
		if (_smoke == 2) then { waitUntil { sleep 1; count (getMarkerPos "mkrLZ" nearObjects ["SmokeShellRED",250]) > 0; }; };
		if (_smoke == 3) then { waitUntil { sleep 1; count (getMarkerPos "mkrLZ" nearObjects ["SmokeShellYELLOW",250]) > 0; }; };
		if (_smoke == 4) then { waitUntil { sleep 1; count (getMarkerPos "mkrLZ" nearObjects ["SmokeShellPURPLE",250]) > 0; }; };
		if (_smoke == 5) then { waitUntil { sleep 1; count (getMarkerPos "mkrLZ" nearObjects ["SmokeShellGREEN",250]) > 0; }; };
		if (_smoke == 6) then { waitUntil { sleep 1; count (getMarkerPos "mkrLZ" nearObjects ["SmokeShellORANGE",250]) > 0; }; };
		sleep 1;
		if (_smoke == 1) then { _posSMK = getPos (getPos player nearestObject "SmokeShellBLUE"); };
		if (_smoke == 2) then { _posSMK = getPos (getPos player nearestObject "SmokeShellRED"); };
		if (_smoke == 3) then { _posSMK = getPos (getPos player nearestObject "SmokeShellYELLOW"); };
		if (_smoke == 4) then { _posSMK = getPos (getPos player nearestObject "SmokeShellPURPLE"); };
		if (_smoke == 5) then { _posSMK = getPos (getPos player nearestObject "SmokeShellGREEN"); };
		if (_smoke == 6) then { _posSMK = getPos (getPos player nearestObject "SmokeShellORANGE"); };
		
		// 2nd inv helipad at smoke
		_smkLZ = createVehicle ["Land_HeliPadEmpty_F", _posSMK, [], 0, "NONE"];
		sleep 1;
	};
