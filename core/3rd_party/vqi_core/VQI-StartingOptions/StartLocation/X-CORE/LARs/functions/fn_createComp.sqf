...
//[ COMP_NAME, POS_ATL, OFFSET, DIR, ALIGN_TERRAIN, ALIGN_WATER ] call LARs_fnc_spawnComp;
...
_msg = format[ "COMP - Name: %1, Pos:%2, Offset: %3, Rot: %4, Align: %5, Water: %6", _compName, _compPos, _compOffset, _compRot, _compAlign, _compWater ];
...
if ( DEBUG_DEV ) then {
	...
	_arrow = createVehicle [ "Sign_Arrow_Direction_Green_F", ASLToATL _compPos vectorAdd [ 0, 0, 3 ], [], 0, "CAN_COLLIDE" ];
...
private _fnc_setPositionAndRotation = {
	...
	private _objAsPlaced = _asPlaced;
...
//	};	
	...
	_P = 360 - deg _P;
	_R = 360 - deg _R;
diag_log format[ "Y: %1, P: %2, R: %3", _Y, _P, _R ];
	...
	if !( _compRot isEqualTo 0 ) then { _objAsPlaced = false };
	
diag_log format[ "asPlaced : %1", _objAsPlaced ];

	if !( _objAsPlaced ) then {
		
		if !( _compRot isEqualTo 0 ) then {
			...
			_cfgOffset = _cfgOffset vectorAdd _compOffset;
		};
		
		_newPosX = ( _compPos select 0 ) + ( _cfgOffset select 0 );
		_newPosY = ( _compPos select 1 ) + ( _cfgOffset select 1 );
		_newPosZ = ( _compPos select 2 ) + ( _cfgOffset select 2 );
		_pos = [ _newPosX, _newPosY, _newPosZ + _ATLOffset ];
		
		_objZOffset = abs( boundingBoxReal _obj select 0 select 2 );
		
		//If using placement radius get position
		if ( _placementRadius > 0 ) then {
			_pos = AGLToASL ( _pos getPos [ random _placementRadius, random 360 ] );
			_pos set [ 2, _objZOffset + ( _cfgOffset select 2 ) ];
		};
		
		//Work out any terrain/water alignment
		if ( surfaceIsWater _pos && _compWater ) then {
			_pos set [ 2, ( _cfgOffset select 2 ) ];
		}else{
			if ( _compAlign || _needsSurfaceUp ) then {
		...
		//If using a random start pos choose a position
		if ( count _randomStartPos > 0 ) then {
			_pos = selectRandom ( [ _pos ] + _randomStartPos );
		};
		
		//*********
diag_log format[ "align : %1, ca: %2, nsu: %3, wtr: %4",
	( _compAlign || _needsSurfaceUp ) && !( surfaceIsWater _pos && _compWater ),
	_compAlign,
	_needsSurfaceUp,
	_compWater
];

		if ( ( _compAlign || _needsSurfaceUp ) && !( surfaceIsWater _pos && _compWater ) ) then {
			
			//Get composition as placed pitch and roll
			_pbO = _obj call BIS_fnc_getPitchBank;
			_pbO params[ "_origP", "_origR" ];		
		
			_obj setVectorUp surfaceNormal _pos;
			
			_pbN = _obj call BIS_fnc_getPitchBank;
			_pbN params[ "_pbP", "_pbR" ];
...
diag_log format[ "Y: %1, P: %2, R: %3", _Y, _P, _R ];
		...
		//As placed default position and rotation
...
private _fnc_CustomAttributes = {
	...
	
	{
		...
		if ( _expression find "%s" > -1 ) then {
			...
			_expression = format[ "%1%2%3", _split select 0, _property, _split select 2 ];
		};
...
private _fnc_spawnLogic = {
	...
	if ( random 1 <= _presence && { call compile _preCondition } ) then { 
		...
		//FIX: Seems to not to be saved in a composition sqe ??
		//_placementRadius = getNumber( _cfg >> "Attributes" >> "placementRadius" );
		
		_category = getText( configFile >> "CfgVehicles" >> _type >> "category" );
