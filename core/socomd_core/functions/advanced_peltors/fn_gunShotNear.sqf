 ...
 * Modified by Slatery
 * Handles weapons going off near the player.
 *
 * Arguments:
 ...
 *
 * Example:
 * [clientFiredNearEvent] call ace_peltors_fnc_firedNear
...
private _vehAttenuation = if ((ACE_player == (vehicle ACE_player)) || {isTurnedOut ACE_player}) then {1} else {EGVAR(hearing,playerVehAttenuation)};
...
private _loudness = EGVAR(hearing,cacheAmmoLoudness) getVariable (format ["%1%2",_weapon,_ammo]);
if (isNil "_loudness") then {
    ...
    EGVAR(hearing,cacheAmmoLoudness) setVariable [(format ["%1%2",_weapon,_ammo]), _loudness];
};
